  ...
  <div id="scrollImg">
    <div class="outDiv">
      <div class="innerDiv" data-img="true">
        <img :src='imgSrc' class="targetImg" />
      </div>
    </div>
  </div>
</template>
    ...
    scrollImgInit() {
      ...
      function LongPicShow(options) {
        this.config = {
          ...
          speed: 350 // 默认为150 值越小 执行的越慢     time = 图片height/speed
        };
          ...
          $(_config.targetImg).each(function(index, item) {
            ...
            $($(tagParent).find("div")[1]).hover(
              function() {
                ...
                $(item).animate(
                  { top: -$imgHeight + $(tagParent).height() },
                  $time * 1000,
                  "linear"
                );
              },
